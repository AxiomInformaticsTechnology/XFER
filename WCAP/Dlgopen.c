...
BOOL GetFileName (HWND hWndOwner, LPSTR szFileName, PDWORD_PTR dwFlags)
{
    ...
    if (GetSaveFileName (&of))
    {
        ...
        {
          int cnt = strlen(of.lpstrFile);
          cnt     = (cnt>=MAXFILENAMELEN-2) ? MAXFILENAMELEN-2 : cnt  ;
          lstrcpyn(szFileName, of.lpstrFile, cnt+1); //including room for the terminating ANSI 0
//        szFileName[cnt  ] = '\0';
        }
        // Copy flags out
        (long)*dwFlags = of.lCustData;
...
BOOL APIENTRY FileSaveHookProc (HWND hDlg, WORD msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg)
    {
        case WM_INITDIALOG:
            ...
            (long)dwFlags = lpOpenFn->lCustData; 
