...
/*DWORD           BufferSize  = 4096;
  ...
  DWORD           dwUsage     = 0;  //all resources
  NETRESOURCE     NetResource = { RESOURCE_GLOBALNET, RESOURCETYPE_ANY, RESOURCEDISPLAYTYPE_GENERIC, 0, NULL, NULL, NULL, NULL };
  ...
  HANDLE          hEnum;
  DWORD           vnoe  = WNetOpenEnum(dwScope, dwType, dwUsage,&NetResource,&hEnum );
  DWORD           vner  = WNetEnumResource(hEnum,&cCount, &lpBuffer[0],&BufferSize);
  DWORD           vnce  = WNetCloseEnum(hEnum);
 
  DWORD           vngc  = WNetGetConnection("\\\\HELIOS\\ITS", &lpBuffer[0],&cnt);
  switch(vngc)
  {
    case NO_ERROR:
      vngc = 0;
  ...
  */
  strncpy(szAppName, "WinCap", 7);     // Name of our App
	szAppName[6] = '\0';
            ...
            switch (LOWORD(wParam))
            {
                ...
                case IDM_VIEWCLEAR:
                {
                    ...
                    if (gbSave)
                    {
                        ...
                        wRet =(WORD) MessageBox(hWnd, lpBuffer,
...
void SaveMe()
{
    ...
    dwFlags = (DWORD_PTR)MAKELONG(wCompression, wBitCount);


    // Bring up File/Save... dialog and get info. about filename,
    // compression, and bits/pix. of DIB to be written.

    bResult = GetFileName(ghWndMain, (LPSTR)szFileBuf, &dwFlags);  // Function will accept up to 255 characters.
...
void PrintMe()
{
    ...
    iReturn = (DWORD_PTR)DialogBoxParam(ghInst, (LPSTR)"Print", ghWndMain, PrintDlgProc, (LPARAM) &opts);
...
void DoCapture(HWND hWnd, WORD wCommand)
{
    ...
    switch (wCommand)
    {
        ...
        case IDM_DESKTOP:
        {
            ...
            strncpy(gszWindowText, lpBuffer, sizeof(gszWindowText) - 1);
			gszWindowText[sizeof(gszWindowText) - 1] = '\0';
...

            {
              ...
              TPG             = (DWORD)(duration*CLOCKS_PER_SEC);
              m_TimePerGrab   = (DWORD)TPG;
            }
        ...
        case IDM_ACTIVEWINDOW:
        {
            ...
            if (hWndSelect == hWndDesktop)
            {
                ...
                HDC     hDC;        // DC to screen
                MSG     msg1;       // Message for the PeekMessage()
                ...
                while (PeekMessage(&msg1,NULL,0,0,PM_REMOVE) != 0)
                {
...
void DrawSelect(HDC hdc, BOOL fDraw, LPRECT lprClip)
{
    ...
    if (!IsRectEmpty(&rcClip))
    {
        ...
        _snprintf(sz, sizeof(sz) - 1, "%dx%d", rcClip.right - rcClip.left, rcClip.bottom -
               rcClip.top);
		sz[sizeof(sz) - 1] = '\0';
